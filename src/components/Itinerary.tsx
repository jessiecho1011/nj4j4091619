...
export default function Itinerary({ days }: ItineraryProps) {
  ...
  return (
    <section id="itinerary" className="py-20 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto scroll-mt-12">
      ...
      {/* 時間軸本體 */}
      <div className="relative border-l border-teal-200 ml-4 sm:ml-32 space-y-12">
        {days.map((day) => (
          <div key={day.id} className="relative pl-6 sm:pl-10 group">
            ...
            {/* 每日行程卡片 */}
            <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-xs border border-slate-100 hover:shadow-lg hover:border-teal-100 hover:-translate-y-0.5 transition-all duration-300">
              {/* 卡片標頭 */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4 mb-4">
                <div>
                  <div className="sm:hidden text-xs font-semibold text-teal-500 mb-1">
                    Day {day.day} • {formatDate(day.date)}
                  </div>
                  <h3 className="text-lg sm:text-xl font-bold text-slate-800 group-hover:text-teal-600 transition-colors">
                    {day.title}
                  </h3>
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {day.spots.map((spot, spotIndex) => (
                    <div
                      key={spotIndex}
                      className="flex items-start gap-3 p-3 rounded-xl bg-slate-50/60 border border-slate-100/80 hover:bg-teal-50/30 hover:border-teal-100/50 transition-all group/spot"
                    >
                      ...
                      <div>
                        <div className="text-sm font-semibold text-slate-700 group-hover/spot:text-teal-950 transition-colors">
                          {spot}
                        </div>
